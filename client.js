/* 
이 코드는 TCP 클라이언트를 생성하여 서버와 연결하고, 패킷을 전송 및 수신하는 기능을 구현합니다.
클라이언트는 초기화 패킷을 전송하고, 서버로부터 응답을 받아 처리합니다.
또한, 방을 생성하고 생성된 게임 ID를 파일에 저장하는 기능도 포함되어 있습니다.
위치 업데이트를 주기적으로 서버에 전송하는 기능도 포함되어 있습니다.
*/
...
const saveGameId = (gameId) => {
  fs.writeFileSync('gameId.json', JSON.stringify({ gameId }), 'utf8');
};
...
client.connect(PORT, HOST, async () => {
  ...
  await sendPacket(client, createGamePacket);  // 게임 생성 패킷 전송
});
  ...
  if (packetType === 1) {
    ...
    try {
      ...
      if (responseData.gameId) {
        gameId = responseData.gameId;  // 응답에서 게임 ID 가져오기
        saveGameId(gameId);  // 파일에 게임 ID 저장
      }
    } catch (e) {
    ...
    try {
      ...
      sendPong(client, timestampLong.toNumber());  // Pong 응답 전송
    } catch (pongError) {
